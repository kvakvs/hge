...
hgeGUIText::hgeGUIText(int _id, float x, float y, float w, float h,
		hgeFont *fnt)
{
	id = _id;
	bStatic = true;
...
hgeGUIButton::hgeGUIButton(int _id, float x, float y, float w, float h,
...
hgeGUISlider::hgeGUISlider(int _id, float x, float y, float w, float h,
		HTEXTURE tex, float tx, float ty, float sw, float sh, bool vertical)
{
	id = _id;
	bStatic = false;
	bVisible = true;
...
hgeGUIListbox::hgeGUIListbox(int _id, float x, float y, float w, float h,
		hgeFont *fnt, uint32_t tColor, uint32_t thColor, uint32_t hColor)
{
	id = _id;
	bStatic = false;
	bVisible = true;
...
void hgeGUIListbox::Render()
{
	...
	for (i = 0; i < GetNumRows(); i++)
	{
		...
		font->Render(rect.x1 + 3, rect.y1 + i * font->GetHeight(),
				HGETEXT_LEFT, pItem->text);
		pItem = pItem->next;
	}
}
