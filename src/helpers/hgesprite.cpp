...
void hgeSprite::Render4V(float x0, float y0, float x1, float y1, float x2,
...
hgeRect* hgeSprite::GetBoundingBoxEx(float x, float y, float rot, float hscale,
		float vscale, hgeRect *rect) const
{
	float tx1, ty1, tx2, ty2;
	float sint, cost;
	...
	if (rot != 0.0f)
	{
		// TODO: change this to fsincos intrinsic or something like that
		cost = cosf(rot);
		sint = sinf(rot);
