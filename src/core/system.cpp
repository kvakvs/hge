...
bool HGE_CALL HGE_Impl::System_Initiate()
{
	...
	if (szIcon)
		winclass.hIcon = HGE_WINAPI_UNICODE_SUFFIX(LoadIcon)(hInstance, szIcon);
	else
	...
	height = nScreenHeight + GetSystemMetrics(SM_CYFIXEDFRAME) * 2
	...
	if (bWindowed)
		hwnd = HGE_WINAPI_UNICODE_SUFFIX(CreateWindowEx)(0, WINDOW_CLASS_NAME,
				szWinTitle, styleW, rectW.left, rectW.top, rectW.right
						- rectW.left, rectW.bottom - rectW.top, hwndParent,
				NULL, hInstance, NULL);
	else
		hwnd = HGE_WINAPI_UNICODE_SUFFIX(CreateWindowEx)(WS_EX_TOPMOST,
				WINDOW_CLASS_NAME, szWinTitle, styleFS, 0, 0, 0, 0, NULL, NULL,
				hInstance, NULL);
	if (!hwnd)
	{
...
void HGE_CALL HGE_Impl::System_Shutdown()
{
	...
	if (hInstance)
		HGE_WINAPI_UNICODE_SUFFIX(UnregisterClass)(WINDOW_CLASS_NAME, hInstance);
		...
		hgeConstString value)
{
	...
	case HGE_ICON:
		...
		if (pHGE->hwnd)
		{
			HGE_WINAPI_UNICODE_SUFFIX(SetClassLong)(pHGE->hwnd, GCL_HICON,
					(LONG) HGE_WINAPI_UNICODE_SUFFIX(LoadIcon)(pHGE->hInstance,
							szIcon));
		}
		break;

	case HGE_TITLE:
		hge_strcpy(szWinTitle, value);
		if (pHGE->hwnd)
		{
	...
	case HGE_INIFILE:
		if (value)
			hge_strcpy(szIniFile, Resource_MakePath(value));
		else
			szIniFile[0] = 0;
		break;
...
{
	if ((uint32_t) HGE_WINAPI_UNICODE_SUFFIX(ShellExecute)(pHGE->hwnd, NULL,
...
HGE_Impl::HGE_Impl()
{
	...
	int i;
	for (i = hge_strlen(szAppPath) - 1; i > 0; i--)
		if (szAppPath[i] == '\\')
			break;
	szAppPath[i + 1] = 0;
}
