...
bool DoCommands(const int id) {
    ...
    case CMD_FAMILYLIST:
        ...
        SliderSetValue(CMD_FAMILYSLIDER, static_cast<float>(listbox->GetTopItem()));
        cmdGenerateFont();
        break;
    ...
    case CMD_CHARRANGE:
        static_cast<hgeGUIRange*>(gui->GetCtrl(CMD_CHARRANGE))->GetRange(&f, &l);
        state.sr.First = f;
        state.sr.Last = l;
        cmdGenerateFont();
        break;
...
void cmdGenerateFont() {
    ...
    texFont = FontGenerate(state.FontFamily, state.nSize, state.nPadTop, state.nPadBtm,
                           state.nPadLft,
