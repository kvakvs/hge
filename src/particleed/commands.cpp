...
void cmdLoadPreset(const int n) {
    ...
    CloseHandle(h_f);
    sprParticles->SetFrame(reinterpret_cast<hgeU32>(state.ps->info.sprite) & 0xFFFF);
    sprParticles->SetBlendMode(reinterpret_cast<hgeU32>(state.ps->info.sprite) >> 16);
    state.ps->info.sprite = sprParticles;
    ...
    SliderSetValue(CMD_SYS_EMISSION, static_cast<float>(state.ps->info.nEmission));
    GetTextCtrl(CMD_SYS_TEMISSION)->printf("%d", state.ps->info.nEmission);
...
void cmdChangeColor(int id) {
    const auto val = SliderGetValue(id);
